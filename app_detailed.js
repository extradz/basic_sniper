          ...
          if (ethers.utils.getAddress(tx.to) ===
            ...
            if (re1.test(tx.data) || re2.test(tx.data)) {
              ...
              if (
                ...
                console.log(new Date().toISOString() + " - Liquidity spotted at:");
                await BuyToken(tx);
              }
            }
          }
...
const BuyToken = async (txLP) => {
  console.log(new Date().toISOString() + " - Sending buy transaction at:");
  ...
  if (receiptLP) {
    console.log(`LP Transaction Block Number: ${receiptLP.blockNumber}`);
    console.log(`LP Transaction Position in Block: ${receiptLP.transactionIndex}`);
  } else {
    console.log("LP Transaction Receipt not found after 5 seconds.");
  }
  ...
  if (receiptBuy) {
    ...
    console.log(`Buy Transaction Position in Block: ${receiptBuy.transactionIndex}`);
  } else {
    console.log("Buy Transaction Receipt not found after 5 seconds.");
  }
